...
#define FITEPOCH 631065600
			time_t now = FITEPOCH+record->timestamp;
			char   stamp[32];

			strftime(stamp, sizeof(stamp), "%m/%d/%Y-%H:%M:%S", localtime(&now));


			printf("%20.20s\t%lu", stamp, FITEPOCH+record->timestamp);
			printf("\t%f", record->position_lat*180.0/0x7FFFFFFF);
			printf("\t%f", record->position_long*180.0/0x7FFFFFFF);
#define FEETPERMETER 3.28084
			printf("\t%f", FEETPERMETER*(record->altitude/5.0-500));
#define CMPERMILE 160934.0
			printf("\t%6.3f", record->distance/CMPERMILE);
#define MMS2MPH 447.04
			printf("\t%6.3f", record->speed/MMS2MPH);
			printf("\t%hhu", record->heart_rate);
			printf("\t%hu", record->power);
			printf("\t%hhu", record->cadence);
			printf("\n");
